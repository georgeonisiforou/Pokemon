...
const PokemonWithReactQuery = () => {
  ...
  //calculate the rotation of the card on mouse hover
  ...
  //fetch all pokemon of the selected generation
  const fetchAllPokemon = async () => {
    const result = await axios.get(
      `https://pokeapi.co/api/v2/generation/${generation}`
    );
    return result.data.pokemon_species;
  };
  ...
  //refetch all pokemon when new generation is selected
  useEffect(() => {
    allPokemonQuery.refetch();
  }, [generation]);
  ...
  const fetchPokemonData = async (pokemonURL) => {
    const resultData = await axios.get(pokemonURL);
    return resultData;
  };
  ...
  const selectedPokemonQuery = useQuery({
    ...
    queryFn: fetchPokemonData(selected),
  });
    ...
    <Container>
      ...
      >
        {allPokemonQuery.data &&
          allPokemonQuery.data.map((pokemon, idx) => {
            return (
...
export default PokemonWithReactQuery;
